...
╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚═╝        ██║1.1
    ...
    # --- Enumeration / Cache Generation Operations ---
        ...
        attributes_to_fetch = []
        domain_root_dn = "" # Initialize here, will be populated if --cache
        if options.cache:
        ...
        if options.cache:
            # Call the cache generation and combination functions from the module
            id_to_type_cache_dict, value_to_id_cache_dict = cache_generator.generate_soaphound_caches(
                all_pulled_items, domain_root_dn
            )
            if id_to_type_cache_dict and value_to_id_cache_dict:
                cache_generator.combine_generated_caches(
                    id_to_type_cache_dict, value_to_id_cache_dict
                )
        elif options.bloodhound:
