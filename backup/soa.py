    ...
    if options.rbcd != None:
        ...
        if options.bloodhound:
            attributes = [
                    ...
                    "msDS-AllowedToDelegateTo", "gPCFileSysPath", "gPLink", "gPOptions"
                ]
        elif options.cache:

            attributes = ["objectSid", "objectGUID", "distinguishedName"]

        elif options.filter is not None:
                attributes = [x.strip() for x in options.filter.split(",")]                
        ...
        sys.stdout = old_stdout  # Restauration de la sortie standard

        output = captured_stdout.getvalue()
        lines = output.splitlines()

        results = {"ValueToIdCache": {}, "IdToTypeCache": {}}
        current_dn = None
        current_sid = None
        current_guid = None
        ...
        object_class_mapping = {
            ...
            "computer": 1,
            "gPLink": 5,
            "groupPolicyContainer": 6,
            "msDS-GroupManagedServiceAccount": 2,
        }

        for line in lines:
            line = line.strip()
            if line.startswith("distinguishedName: "):
                current_dn = line.split("distinguishedName: ")[1].strip()
            elif line.startswith("objectSid: "):
                current_sid = line.split("objectSid: ")[1].strip()
                if "Well known sid:" in current_sid:
                    current_sid = current_sid.split("Well known sid:")[0].strip()
            elif line.startswith("objectGUID: "):
                current_guid = line.split("objectGUID: ")[1].strip()
            elif line.startswith("objectClass: "):
                current_object_class = line.split("objectClass: ")[1].strip()

            if current_dn:
                if current_sid:
                    results["ValueToIdCache"][current_dn] = current_sid
                elif current_guid:
                    results["ValueToIdCache"][current_dn] = current_guid

            if current_guid and current_object_class:
                if current_object_class in object_class_mapping:
                    results["IdToTypeCache"][current_guid] = object_class_mapping[current_object_class]

            # Réinitialiser seulement si current_dn est None, indiquant un nouvel objet
            if current_dn is None and current_sid is None and current_guid is None and current_object_class is None:
                current_dn = None
                current_sid = None
                current_guid = None
                current_object_class = None
        ...
        if options.cache:
            ...
            for key, value in results["ValueToIdCache"].items():
                normalized_key = normalize_utf8(key)
                normalized_valuetoid[normalized_key] = value

            results["ValueToIdCache"] = normalized_valuetoid
